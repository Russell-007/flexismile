...
export const getTreatmentPlans = async (patientId) => {
  if (USE_MOCK) {
    ...
    try {
      const stored = localStorage.getItem(`plans-${patientId}`);
      if (!stored) return [];

      const parsed = JSON.parse(stored);
      return Array.isArray(parsed) ? parsed : [];
    } catch {
      localStorage.removeItem(`plans-${patientId}`);
      return [];
    }
  }

  const res = await axios.get(`${API_URL}/treatment-plans/${patientId}`);
...
/* =========================
   CREATE PLAN
========================= */
export const createTreatmentPlan = async (plan,patientId) => {
  if (USE_MOCK) {
    await wait(500);
    ...
    const newPlan = {
      ...
      ipr : plan.ipr.map((i) => i.preview),
    };

    const existing = JSON.parse(localStorage.getItem(`plans-${patientId}`) || "[]");
  ...
  plan.ipr.forEach((i) => i.file && formData.append("ipr", i.file));
...
/* =========================
   UPDATE PLAN
========================= */
export const updateTreatmentPlan = async (plan,patientId) => {
  if (USE_MOCK) {
    await wait(500);
    ...
    const updated = existing.map((p) =>
      p.id === plan.id
        ? {
            ...
            ipr : plan.ipr.map((i)=> i.preview || i),
          }
        : p
    );

    localStorage.setItem(`plans-${patientId}`, JSON.stringify(updated));

    return updated.find(p => p.id === plan.id);
  }
...
export const deleteTreatmentPlan = async (planId,patientId) => {
  if (USE_MOCK){
    await wait(500);
